...
// returns whether two PCI structures are identicle
// helpful function if you don't have a pointer to a pci device.
bool pci_compare(pci_dev_t dev_a, pci_dev_t dev_b) {
	return (
		dev_a.bits 				== dev_b.bits 			&&
		dev_a.field_num 		== dev_b.field_num 		&&
		dev_a.function_num 		== dev_b.function_num 	&&
		dev_a.device_num 		== dev_b.device_num 	&&
		dev_a.bus_num 			== dev_b.bus_num 		&&
		dev_a.reserved 			== dev_b.reserved 		&&
		dev_a.enable 			== dev_b.enable
	);
}
...
uint32_t pci_read(pci_dev_t dev, uint32_t field) {
	...
	dev.enable = 1;
	Ports::outl(PCI_CONFIG_ADDRESS, dev.bits);
	...
	if(size == 1) {
		...
		uint8_t t = Ports::inb(PCI_CONFIG_DATA + (field & 3));
		return t;
	}
	else if(size == 2) {
		uint16_t t = Ports::inw(PCI_CONFIG_DATA + (field & 2));
		return t;
	}
	else if(size == 4){
		// Read entire 4 bytes
		uint32_t t = Ports::inl(PCI_CONFIG_DATA);
		return t;
	}
	return 0xffff;
}
...
pci_dev_t pci_get_device(uint16_t vendor_id, uint16_t device_id, int device_type) {
	...
	if(pci_reach_end(dev_zero)) {
		term.printf("PCI Get device failed...\n");
	}
	for(int function = 1; function < FUNCTION_PER_DEVICE; function++) {
		pci_dev_t dev = {0};
		dev.function_num = function;
...
void pci_init() {
	...
	dev_zero.bits = 0; // bits
	dev_zero.field_num = 0; // field_num
	dev_zero.function_num = 0; // function_num
	dev_zero.device_num = 0; // device_num
	dev_zero.bus_num = 0; // bus_num
	dev_zero.reserved = 0; // reserved
	dev_zero.enable = 0;  // enable
}